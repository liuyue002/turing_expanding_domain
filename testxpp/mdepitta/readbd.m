...
opts.mergefirst = 0; % Orbits only: join 1st point of the set with last of previous set / set it to 0 to turn it off
opts.mergelast = 0;  % Orbits only: join last point of the set with first of following set / set it to 0 to turn it off
opts.dsmax = 0.1;      % Threshold for change in bifurcation parameter for a datapoint to be considered a new branch
...
while i<=size(data,1)
    ...
    % are the same, AND change in bifurcation parameter is small enough
    while (j<=size(data,1))&&(data(j,1)==data(index0,1) && data(j,2)==data(index0,2) && abs(data(j,opts.par1)-data(j-1,opts.par1)) < opts.dsmax)
        j = j+1;
    end
